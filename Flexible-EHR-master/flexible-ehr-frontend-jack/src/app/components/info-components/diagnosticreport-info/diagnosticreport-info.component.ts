...
import {DiagnosticreportService, Report} from '../../../services/diagnosticreport.service';
...
@Component({
  selector: 'app-diagnosticreport-info',
  templateUrl: './diagnosticreport-info.component.html',
  styleUrls: ['./diagnosticreport-info.component.scss']
})
export class DiagnosticreportInfoComponent implements OnInit {
  ...
  onSelect($event: MatAutocompleteSelectedEvent) {
    $event.source.options.forEach(async (value, i) => {
      if (value.selected) {
        ...
        this.diagnosticReports = await this.diagnosticreportService.searchByPatientId(this.selectedPatient.id)
      }
    })
  ...
  add() {
    ...
    this.dialogRef.close({
      ...
      data: this.diagnosticReports.filter(x => x.isSelected),
      parameters: {
        patientId: this.selectedPatient.id,
        diagnosticReport: this.diagnosticReports.filter(x => x.isSelected).map(x => x.id)
      }
    })
  }
