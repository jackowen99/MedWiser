...
export class ImmunizationService {
  ...
  async searchByPatientId(id: string): Promise<Immunizations[]> {
    let result = [];
  ...
  async searchByPatientId_hapi(id: string): Promise<Immunizations[]> {
    if (id == '' || id == null) {
      return [];
    }
    const result = await this.http.get<any>('http://hapi.fhir.org/baseR4/Immunization', {
      params: {
        patient: id
      }
    }).toPromise().catch(e => {
